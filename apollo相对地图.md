...
​	导航模式的基本思路是**通过人工驾驶方式录制测试道路上的行驶轨迹**，利用Apollo工具对原始轨迹进行处理得到平滑轨迹，即为Reference line或称指引线、中心线、参考线。

​	自动驾驶过程中，感知模块的相机动态检测道路边界及障碍物，地图模块下的相对地图子模块基于参考线及道路边界**实时地生成相对地图**，规划模块依据地图模块输出的相对地图和感知模块输出的障碍物信息，动态输出局部行驶路径给控制模块执行。
